...
let finishHimPlayed = false; // play finish-him.mp3 only once
...
const finishHimAudio = new Audio('finish-him.mp3');
finishHimAudio.volume = 0.9;

function playPunch() {
  // Clone node so rapid taps can overlap each other
  const clone = punchAudio.cloneNode();
  clone.volume = 0.7;
  clone.play().catch(() => {}); // silently ignore if browser blocks autoplay
}

function playFinishHim() {
  if (finishHimPlayed) return;
  finishHimPlayed = true;
...
function punch() {
  ...
  // Check 80% threshold for finish-him
  const pct = Math.round((Math.min(punches, MAX_PUNCHES) / MAX_PUNCHES) * 100);
  if (pct >= 80) playFinishHim();
}
...
function resetPunches() {
  ...
  combo   = 0;
  damageLevel = 0;
  finishHimPlayed = false;
  clearTimeout(comboTimer);

  // Stop finish-him audio if playing
  finishHimAudio.pause();
  finishHimAudio.currentTime = 0;
